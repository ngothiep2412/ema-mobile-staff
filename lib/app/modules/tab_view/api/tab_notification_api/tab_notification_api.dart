...
class TabNotificationApi {
  static Future<List<NotificationModel>> getAllNotification(
      String jwtToken, int page) async {
    var response = await http.get(
      Uri.parse(
          '${BaseLink.localBaseLink}${BaseLink.getAllNotification}?sizePage=10&currentPage=$page'),
      headers: {
    ...
    if (response.statusCode == 201 || response.statusCode == 200) {
      ...
      listNotifications.addAll(jsonData
          .map((listNotifications) =>
              NotificationModel.fromJson(listNotifications))
  ...
  static Future<ResponseApi> seenANotification(
      String jwtToken, String notificationID) async {
    var response = await http.put(
      Uri.parse(
          '${BaseLink.localBaseLink}${BaseLink.seenANotification}?notificationId=$notificationID'),
      headers: {
        "Accept": "application/json",
        "content-type": "application/json",
        'Authorization': 'Bearer $jwtToken',
